...
against internals. Tests verify behavior through public interfaces, not
implementation details — code can change entirely; tests shouldn't. A
...
seams under test and confirm them with the user (the plan's Testing
Decisions section may already record them). You can't test everything —
agreeing the seams up front is how testing effort lands on the critical
...
- **Tautological** — the assertion recomputes the expected value the way
  ...
  come from an independent source of truth — a known-good literal, a
...
- **Horizontal slicing** — writing all tests first, then all
  ...
  *shape* of things rather than user-facing behavior, the tests go
  ...
  one test → one implementation → repeat, each test a **tracer bullet**
...
Next failing test for next feature — one seam, one test, one minimal
...
[writing-good-tests.md](writing-good-tests.md) for the rules that keep
tests honest:
...
See [tests.md](tests.md) for worked examples of behavior-driven tests and
[mocking.md](mocking.md) for when mocking is and isn't appropriate.
...
- Tests added "later"
- Test written at a seam nobody agreed to
...
- [ ] Every test sits at a pre-agreed seam
